...
class ModelArchitecture(BaseModel):
    ...
    layers: Optional[List[str]] = Field(None, description="List of layers used in the model architecture, e.g., ['Conv2D', 'MaxPooling', 'Dense'].")
    neurons_per_layer: Optional[List[conint(ge=1)]] = Field(None, description="Number of neurons in each layer, corresponding to the layers list.")
...
class OptimizationSettings(BaseModel):
    ...
    loss_function: str = Field(..., description="Loss function used for training, e.g., 'categorical_crossentropy', 'mean_squared_error'.")
...
class RegularizationTechniques(BaseModel):
    ...
    other_techniques: Optional[List[str]] = Field(None, description="List of other regularization techniques applied, e.g., ['L2 regularization', 'Batch normalization'].")
