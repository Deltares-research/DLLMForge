...
TEST_INDEXES = [
    "deltares_rag_demo_chunks_100_overlap_10_ada002", "deltares_rag_demo_chunks_100_overlap_10_text_embedding_3_large",
    "deltares_rag_demo_chunks_300_overlap_30_ada002"
]
...
class DeltaresRAGASTest:
    ...
    def create_retriever(self, index_name: str) -> Retriever:
        ...
        try:
            ...
            retriever = Retriever(embedding_model, index_name, self.search_endpoint, self.search_api_key)
    ...
    def test_single_index(self, index_name: str, questions: List[Dict[str, str]]) -> Dict[str, Any]:
        ...
        for i, q_data in enumerate(questions, 1):
            ...
            try:
                ...
                # Evaluate with RAGAS
                evaluation = evaluate_rag_response(question=question,
    ...
    def print_comparison(self, results: Dict[str, Any]):
        ...
        # Detailed header
        print(
            f"{'Configuration':<25} {'Chunk':<8} {'Overlap':<8} {'Model':<20} {'RAGAS':<8} {'Ctx Rel':<8} {'Ctx Recall':<10} {'Faith':<8} {'Ans Rel':<8}"
        )
        print("-" * 100)
        ...
        for index_name, result in results.items():
            ...
            print(
                f"{config_str:<25} {config.get('chunk_size', 'N/A'):<8} {config.get('overlap', 'N/A'):<8} "
        ...
        # Find best performer
        best_index = max(results.keys(), key=lambda x: results[x]['average_scores']['ragas_score'])
        ...
        print(
            f"   Configuration: {best_config.get('chunk_size', 'N/A')} characters, {best_config.get('overlap', 'N/A')} overlap, {best_config.get('model', 'unknown')} model"
        )
        ...
        for i, (index_name, result) in enumerate(sorted_results, 1):
            ...
            config = index_configs.get(index_name, {})
            print(
                f"   {i}. {config.get('chunk_size', 'N/A')} chars, {config.get('overlap', 'N/A')} overlap, {config.get('model', 'unknown')} - RAGAS: {scores['ragas_score']:.3f}"
            )
        ...
        print(
            f"      • Context Relevancy ({best_scores['context_relevancy']:.3f}): How relevant retrieved documents are")
        print(
            f"      • Context Recall ({best_scores['context_recall']:.3f}): How much necessary information was retrieved"
        )
        print(f"      • Faithfulness ({best_scores['faithfulness']:.3f}): Accuracy of answer vs retrieved context")
        print(
            f"      • Answer Relevancy ({best_scores['answer_relevancy']:.3f}): How well answer addresses the question")

        print("\n" + "=" * 100)
        print("🚀 RECOMMENDATIONS")
        print("=" * 100)
        print(
            f"   ✅ Use {best_config.get('chunk_size', 'N/A')}-character chunks with {best_config.get('overlap', 'N/A')}-character overlap"
        )
        print(f"   ✅ Continue using {best_config.get('model', 'unknown')} embedding model for this domain")
        print(
            f"   ✅ System shows excellent faithfulness ({best_scores['faithfulness']:.3f}) - focus on improving retrieval"
        )
